...
select extensions.is(
    (select array_agg(state::text order by state::text) from public.vendor_requests),
...
select extensions.is((select count(*) from public.vendor_requests join sample_snapshot using (id)),
    6::bigint, 'repeat entry preserves all existing sample identifiers');
...
select extensions.is((select count(*) from public.vendor_requests join sample_snapshot using (id)),
    6::bigint, 'failed reset preserves every old request');
select extensions.is((select count(*) from public.vendor_request_audit_events), 18::bigint,
